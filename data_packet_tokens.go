...
func ReadPacketOperandToken(reader *bytes.Reader, token byte) (PacketToken, error) {
	switch {
	case token >= compactOperandFixedBase && token < compactOperandFixedBase+5:
		return PacketToken{
			Kind: PacketTokenFixedOperand,
			Op:   []byte{'h', 'k', 'n', 'g', 'l'}[token-compactOperandFixedBase],
		}, nil
	case token >= compactOperandMagBase && token < compactOperandMagBase+63:
	...
	case token >= compactOperandDirBase && token < compactOperandDirBase+12:
	...
	case token >= compactOperandRotBase && token < compactOperandRotBase+3:
		return PacketToken{
